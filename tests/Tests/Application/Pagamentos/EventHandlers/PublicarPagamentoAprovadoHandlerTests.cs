...
public class PublicarPagamentoAprovadoHandlerTests
{
    private readonly Mock<IOrcamentoGateway> _orcamentoGateway = new();
    private readonly Mock<ISagaEventPublisher> _publisher = new();

    private PublicarPagamentoAprovadoHandler CriarHandler() => new(_orcamentoGateway.Object, _publisher.Object);

    [Fact]
    public async Task Handle_QuandoOrcamentoExiste_DevePublicarPagamentoAprovadoComIdOrdemServico()
    {
        ...
        var domainEvent = new PagamentoAprovadoDomainEvent(Guid.NewGuid(), orcamento.Id, "mp-1");
        ...
        _publisher.Verify(p => p.PublicarPagamentoAprovado(idOrdemServico, domainEvent.IdPagamento, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Handle_QuandoOrcamentoNaoExiste_NaoDevePublicarNada()
    {
        ...
        _publisher.Verify(p => p.PublicarPagamentoAprovado(It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()), Times.Never);
    }
}
